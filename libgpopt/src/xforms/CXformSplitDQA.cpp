	...
	const
{
	...
	if (!(GPOS_FTRACE(EopttraceForceScalarDQAMultiStageAgg) && fScalarDQA)) {
		// if we want to force multi-stage aggregate plan by this traceflag
	...
	if (fScalarDQA && !GPOS_FTRACE(EopttraceForceScalarDQAMultiStageAgg)) {
		// if we want to force multi-stage aggregate plan by this traceflag
