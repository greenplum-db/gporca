...
namespace gpos
{
	...
	class CCache
	{
		...
		// initial value of gclock counter for new entries
			...
			CCacheHashTableEntry *InsertEntry(CCacheHashTableEntry *entry)
			{
			...
			CCacheHashTableEntry *Get(const K key)
			{
				CCacheHashtableAccessor acc(m_hash_table, key);
			...
			void ReleaseEntry(CCacheHashTableEntry *entry)
			{
				GPOS_ASSERT(NULL != entry);
			...
			static
			void DestroyCacheEntryWithRefCountTest(CCacheHashTableEntry *entry)
			{
				GPOS_ASSERT(NULL != entry);
			...
			static
			void DestroyCacheEntry(CCacheHashTableEntry *entry)
			{
				GPOS_ASSERT(NULL != entry);
			...
			ULLONG EvictEntriesOnePass(ULLONG total_freed, ULLONG num_to_free)
			{
